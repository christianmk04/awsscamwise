...
const ProfilePageMain = () => {
  ...
  useEffect(() => {
    const fetchProfileDetails = async () => {
      ...
      try {
        ...
        const profileResponse = await fetch('http://172.31.17.239:5002/get_profile_details/' + userId);
        ...
        const quizzesResponse = await fetch('http://172.31.17.239:5004/get_user_quizzes/' + userId);
        ...
        const savedArticlesResponse = await fetch('http://172.31.17.239:5002/get_saved_articles/' + userId);
        ...
        if (savedArticlesResponseData.length === 0) {
          ...
          const savedArticles = await Promise.all(
            savedArticlesResponseData.map(async (articleId) => {
              const articleResponse = await fetch('http://172.31.17.239:5009/get_news_by_id/' + articleId);
        ...
        const savedQuizzesResponse = await fetch('http://172.31.17.239:5002/get_saved_quizzes/' + userId);
        ...
        const savedQuizzes = await Promise.all(
          savedQuizzesResponseData.map(async (quizId) => {
            const quizResponse = await fetch('http://172.31.17.239:5004/get_quiz_details/' + quizId);
            if (!quizResponse.ok) {
  ...
  const handleRemoveSavedArticle = async (articleId) => {
    try {
      ...
      await fetch('http://172.31.17.239:5002/remove_saved_article', {
        method: 'POST',
  ...
  const handleRemoveSavedQuiz = async (quizId) => {
    try {
      ...
      await fetch('http://172.31.17.239:5002/remove_saved_quiz', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(json_body)
      });
