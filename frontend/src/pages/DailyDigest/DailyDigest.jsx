...
const DailyDigest = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await fetch("http://0.0.0.0:5009/get_all_news");
  ...
  useEffect(() => {
    ...
    if (userId) {
      const fetchBookmarkedArticles = async () => {
        try {
          const response = await fetch(`http://0.0.0.0:5002/get_saved_articles/${userId}`);
          if (!response.ok) {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      if (!recommendationsSent) {
        try {
          const response = await fetch("http://0.0.0.0:5009/get_news_recommendation", {
            method: "POST",
  ...
  const handleBookmark = (articleId) => {
    ...
    const url = isBookmarked
      ? `http://0.0.0.0:5002/remove_saved_article`
      : `http://0.0.0.0:5002/add_saved_article`;
  ...
  const handleDeleteArticle = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://0.0.0.0:5009/delete_news_article/${articleToDelete.newsId}`, {
        method: "DELETE",
      });
