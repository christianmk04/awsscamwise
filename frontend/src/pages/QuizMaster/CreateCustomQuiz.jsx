...
const CreateCustomQuiz = () => {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      const topicEndpoint = 'http://172.31.17.239:5004/get_topics';
      try {
        ...
        showAlert('Failed to fetch topics. Please try again later.', 'error');
      }
    };
    fetchTopics();
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://172.31.17.239:5004/add_quiz", {
        method: "POST",
      ...
      setTimeout(() => navigate("/quiz-master"), 8000);
        ...
        <Container fluid className="welcome-container mx-auto">
          <Row className="align-items-center">
            ...
            <Col xl={5} className='d-flex flex-column justify-content-center'>
              <h1 className='fw-bold mb-4' style={{ color: colors.primary }}>Create Your Own Quiz!</h1>
              <h6 style={{ color: colors.dark, lineHeight: '1.6' }}>
                Play a part in empowering the ScamWise community by designing and submitting a quiz for the community to attempt! 
                Help build a collective understanding of online security, and you could earn experience points!
              </h6>
            </Col>
            <Col xl={5}>
              <DotLottieReact
                src="https://lottie.host/31ea98ff-ee24-4b46-a7fd-f5060b806294/0drckr3yJJ.lottie"
                loop
                autoplay
          ...
          <Form onSubmit={handleSubmit}>
            {activeSection === "quizInfo" && (
              <>
                  ...
                  <Col sm={9}>
                    <Form.Select
                      ...
                      onChange={(e) => {
                        ...
                        setSubTopic("");
                        if (e.target.value !== "Other (Create a New Topic)") {
                          setCustomMainTopic("");
                          setCustomSubTopic("");
                        }
                      }}
                ...
                {/* Custom Main Topic Input */}
                {mainTopic === "Other (Create a New Topic)" && (
                  <Form.Group as={Row} className="mb-4">
                    <Form.Label column sm={3} style={formStyles.formLabel}>
                      Custom Main Topic
                    </Form.Label>
                    <Col sm={9}>
                      <Form.Control
                        type="text"
                        placeholder="Type your main topic here"
                  ...
                  <Col sm={9}>
                    {mainTopic === "Other (Create a New Topic)" ? (
                      ...
                      <Form.Select
                        ...
                        disabled={!mainTopic || !subTopicsArray[mainTopic]}
                        style={formStyles.select}
                      >
                        <option value="">Select a Sub Topic</option>
                        {subTopicsArray[mainTopic]?.map((sub) => (
            ...
            {activeSection === "questions" && (
              <>
                <div style={formStyles.sectionHeader}>
                  <h4 style={{ color: colors.primary, margin: 0 }}>Create Questions</h4>
                </div>
          ...
          {submissionStatus === "loading" ? (
            <>
              ...
              <h4 className="mt-4" style={{ color: colors.primary }}>
                Submitting Quiz... Please hold tight!
              </h4>
            </>
          ) : (
            <>
              ...
              <p className="mt-3" style={{ color: colors.dark }}>
                Your quiz will be reviewed by our admins before it is published.
                <br /><br />
