...
import { Container, Row, Col, Card, Button, OverlayTrigger, Tooltip, Modal, Nav, ProgressBar, Badge, Spinner } from 'react-bootstrap';

const QMDailyPractice = () => {
    ...
    useEffect(() => {
        if (quizDetails.quizId) {
            fetch(`http://0.0.0.0:5006/get_quiz_leaderboard/${quizDetails.quizId}`)
    ...
    useEffect(() => {
        fetch(`http://0.0.0.0:5004/get_daily_quiz`)
            .then(response => response.json())
            .then(data => {
                setQuizDetails({
    ...
    const confirmSubmit = () => {
        ...
        fetch("http://0.0.0.0:5004/new_quiz_submit", {
            method: "POST",
            headers: {
    ...
    if (loading) {
            return (
                <Container className='text-center mt-5'>
                    ...
                    <h3 className='mt-3'>Please wait, your quiz will be available soon...</h3>
                </Container>
            );
    }
...
export default QMDailyPractice;
