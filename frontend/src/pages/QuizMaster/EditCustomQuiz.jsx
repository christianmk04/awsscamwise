...
const EditCustomQuiz = () => {
  ...
  useEffect(() => {
    const loadQuizData = async () => {
      ...
      try {
        ...
        // Fetch quiz details
        const detailsResponse = await fetch(`http://0.0.0.0:5004/get_quiz_details/${quizId}`);
        ...
        const questionsResponse = await fetch(`http://0.0.0.0:5005/get_quiz_questions/${quizId}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://0.0.0.0:5004/edit_quiz", {
        method: "PUT",
