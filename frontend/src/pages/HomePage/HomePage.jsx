  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch user details
        const userResponse = await fetch(`http://0.0.0.0:5002/get_user_details_home/${userId}`);
        if (!userResponse.ok) {
        ...
        const quizResponse = await fetch(`http://0.0.0.0:5004/get_recommended_quizzes_community/${userId}`);
        if (!quizResponse.ok) {
        ...
        const newsResponse = await fetch('http://0.0.0.0:5009/get_news_recommendation', {
          method: 'POST',
          headers: {
