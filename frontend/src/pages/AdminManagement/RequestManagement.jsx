...
const RequestManagement = () => {
  ...
  useEffect(() => {
    const fetchPendingQuizzes = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://172.31.17.239:5004/get_pending_quizzes');
  ...
  const fetchQuizQuestions = async (quizId) => {
    try {
      setLoading(true);
      const response = await fetch(`http://172.31.17.239:5005/get_quiz_questions/${quizId}`);
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      const endpoint = confirmAction === 'accept' 
        ? `http://172.31.17.239:5004/accept_pending_quiz/${selectedQuiz.quizId}`
        : `http://172.31.17.239:5004/reject_pending_quiz/${selectedQuiz.quizId}`;
