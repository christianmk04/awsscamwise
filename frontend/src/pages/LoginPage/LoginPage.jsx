...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('http://0.0.0.0:5001/login_action', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.firstTimeLogin) {
            // Redirect to 0.0.0.0:5173/first-time-details and pass the user ID as a query parameter
            navigate(`/first-time-details`);
            return;
          }

          console.log('Login successful:', data.message);
          navigate(`/home`); // Redirect to 0.0.0.0:5173/
        } else {
