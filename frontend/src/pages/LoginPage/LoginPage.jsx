...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch('http://172.31.17.239:5001/login_action', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.firstTimeLogin) {
            // Redirect to 172.31.17.239:5173/first-time-details and pass the user ID as a query parameter
            navigate(`/first-time-details`);
            return;
          }

          console.log('Login successful:', data.message);
          navigate(`/home`); // Redirect to 172.31.17.239:5173/
        } else {
